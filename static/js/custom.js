...
(function($){
  ...
  NAY.MenuTogglerClose = function(){
	...
	NAY.HeroDarken = function() {
		...
		hero.style.borderRadius = '0 0 ' + currentRadius + 'px ' + currentRadius + 'px';
	...
	NAY.CompetenciesReveal = function() {
		...
		competenciesRaf = window.requestAnimationFrame(function() {
			...
			var progress = scrollRange > 0
				? Math.max(0, Math.min(1, (window.pageYOffset - scrollAreaTop) / scrollRange))
				: 0;
			var botRadius = Math.max(0, Math.min(1, (progress - 0.70) / 0.30)) * maxRadius;

			// Apply to section itself — white page shows through the rounded cutouts
			section.style.borderRadius = topRadius + 'px ' + topRadius + 'px ' + botRadius + 'px ' + botRadius + 'px';
		});
	}
	...
	NAY.CompetenciesScrollAnim = function() {
		...
		function update() {
			...
			var scrollAreaTop = scrollArea.getBoundingClientRect().top + window.pageYOffset;
			var scrollRange = scrollArea.offsetHeight - window.innerHeight;
			if (scrollRange <= 0) { requestAnimationFrame(update); return; }
			...
			if (titleEl) {
				var fillPct = smoothstep(Math.max(0, Math.min(1, progress / 0.25))) * 100;
			...
			for (var i = 0; i < N; i++) {
				...
				var t = smoothstep(Math.max(0, Math.min(1, (progress - itemStart) / skillRevealDur)));
			...
			for (var si = 0; si < statItems.length; si++) {
				...
				var st = smoothstep(Math.max(0, Math.min(1, (progress - statStart) / 0.08)));
			...
			for (var li = 0; li < langChips.length; li++) {
				...
				var lt = smoothstep(Math.max(0, Math.min(1, (progress - chipStart) / 0.06)));
	...
	NAY.HeaderFixd = function() {
		...
		$('#langSwitcherContainer').toggleClass('lang-dark', scrollTop >= heroHeight);
...
NAY.runCounters = function () {
    ...
    * Masonry
    ...
    NAY.navigateTo = function(target) {
      ...
      setTimeout(function() {
        ...
        setTimeout(function() {
          ...
          setTimeout(function() { _navBusy = false; }, 350);
        }, 50);
      }, 350);
    };
    ...
    NAY.changeLanguage = function(lang) {
        ...
        $('.lang-switcher .lang-dropdown a[data-lang="' + lang + '"]').addClass('active');
    ...
    NAY.AboutScrollAnim = function() {
        ...
        function cacheLayoutValues() {
            ...
            if (heroH3) {
                computedH3Size = parseFloat(window.getComputedStyle(heroH3).fontSize) || 100;
            }
        ...
        function updateAnim(dt) {
            ...
            if (Math.abs(springPos - rawTarget) < 0.0005 && Math.abs(springVel) < 0.0005) {
                springPos = rawTarget;
                springVel = 0;
            }
            ...
            var colorProgress = rawColorProgress * rawColorProgress * (3 - 2 * rawColorProgress);
            ...
            var contentProgress = rawContentProgress * rawContentProgress * (3 - 2 * rawContentProgress);

            // -- Phase 1: Background + Title color --
            var bg = lerpStops(bgStops, rawColorProgress);
            sticky.style.backgroundColor = 'rgb(' + bg.r + ',' + bg.g + ',' + bg.b + ')';

            var txt = lerpStops(txtStops, rawColorProgress);
            if (heroH3) heroH3.style.color = 'rgb(' + txt.r + ',' + txt.g + ',' + txt.b + ')';
            if (heroSub) heroSub.style.color = 'rgba(' + txt.r + ',' + txt.g + ',' + txt.b + ',0.6)';
            ...
            if (heroTitle) {
                heroTitle.style.transform = 'translateY(' + currentY + 'px) scale(' + currentScale + ')';
            ...
            if (contentProgress > 0) {
                ...
                contentWrap.style.pointerEvents = contentProgress > 0.3 ? 'auto' : 'none';
                ...
                fadeItems.forEach(function(el, i) {
                    ...
                    var itemP = Math.max(0, (contentProgress - itemDelay) / (1 - itemDelay));
	...
	NAY.ExpertiseWave = function() {
		...
		function draw() {
			...
			for (col = 0; col < COLS; col++) {
				...
				for (row = 0; row < ROWS; row++) {
					...
					yD = amp * (0.58 * Math.sin(p1) + 0.28 * Math.sin(p2) + 0.14 * Math.sin(p3));
					xD = amp * 0.10 * (Math.cos(p1) + 0.35 * Math.cos(p2));
					...
					if (md < 95 && md > 0) {
						mf = (1 - md / 95) * 30;
						sx += (dx / md) * mf; sy += (dy / md) * mf;
					}

					size  = Math.max(0.30, envV * depth * 1.80);
		...
		stickyEl.addEventListener('mouseleave', function() { mouse.x = -9999; mouse.y = -9999; });
	...
	NAY.ExpertiseScroll = function() {
		...
		var n = panels.length, activeIdx = 0;
	...
	NAY.ExperienceTimeline = function() {
		...
		if (reducedMotion) {
			...
			for (var j = 0; j < items.length; j++) items[j].classList.add('exp-active');
			return;
		}
		...
		function updateRay() {
			...
			for (var i = 0; i < items.length; i++) {
				var dotViewY = items[i].getBoundingClientRect().top + 18; /* ~dot offset */
	...
	NAY.CertReveal = function() {
		...
		if (reducedMotion) {
			...
			wall.querySelectorAll('.cert-card').forEach(function(c) { c.classList.add('cert-entered'); });
			return;
		}
		...
		var obs = new IntersectionObserver(function(entries) {
			if (entries[0].isIntersecting) {
				...
				wall.querySelectorAll('.cert-card').forEach(function(c) { c.classList.remove('cert-entered'); });
			}
		}, { threshold: 0.15 });
		obs.observe(section);
	};
	...
	NAY.EducationReveal = function() {
		...
		function positionEduTrack() {
			...
			var trackTop = Math.round(firstNode.offsetTop + firstNode.offsetHeight / 2 - 1);
	...
	NAY.SectionRadius = function() {
		...
		function update() {
			...
			if (footer) {
				...
				var rf = Math.round(Math.min(1, Math.max(0, (vh - driver) / FOOT_RAMP)) * MAX);
